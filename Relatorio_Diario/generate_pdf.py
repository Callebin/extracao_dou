...
class Gerador(FPDF):
    ...
    def add_port(self, portaria, org, destaque, link):
            ...
            self.multi_cell(self.page_width - self.r_margin - self.l_margin, 7, f'{portaria}', 0, align='C')
            
            self.set_text_color(105,105,105)
            self.set_font('Arial', 'B', 10)
            self.multi_cell(self.page_width - self.r_margin - self.l_margin, 7, f'{org}', 0, align='C')
            
            self.set_xy(self.l_margin, self.get_y())
            self.set_font('Arial', '', 9)
            self.multi_cell(self.page_width - self.r_margin - self.l_margin, 7, f'{destaque}', 0, align='C')
            
            self.set_text_color(70,130,180)
            self.set_font('Arial', 'B', 8)
            self.multi_cell(self.page_width - self.r_margin - self.l_margin, 7, f'{link}', 0, align='C')
